...
def test_mid_declares_input_block_3001_3012():
    ...
    assert REG_MID_BLOCK_START == 3001
    assert REG_MID_BLOCK_COUNT == 12
...
def _sample_registers() -> dict[int, int]:
    """One MID block with distinct values per field to make aliasing bugs obvious."""
    return {
        3001: 123,   # L1: 12.3 A
        3002: 45,    # L2: 4.5 A
        3003: 67,    # L3: 6.7 A
        ...
        3010: 0,     # Power reverse (not decoded)
...
def test_decode_polled_ignores_unrelated_addresses():
    regs = _sample_registers()
    regs[999] = 42
    regs[3010] = 9999  # reverse-power address, intentionally not decoded
    result = MidMeterCapability().decode_polled(regs)
